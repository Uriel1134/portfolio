...
export default function ProjectForm({ project, isEdit = false }: ProjectFormProps) {
  const [formData, setFormData] = useState({
    ...
    category: (project?.category as string[]) || [], // Force type string[]
    tech: (project?.tech as string[]) || [], // Force type string[]
  ...
  const [technologies, setTechnologies] = useState<{ name: string; type: string }[]>([]);
  ...
  const fetchConfig = async () => {
    try {
      ...
      const [catRes, techRes] = await Promise.all([
        fetch('/api/admin/config/categories', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('/api/admin/config/technologies', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);

      if (catRes.ok && techRes.ok) {
  ...
  const addCustomTech = async () => {
    ...
    if (formData.tech.includes(name)) {
      setCustomTech('');
      return;
    }
    ...
    try {
      const token = localStorage.getItem('admin-token');
      const response = await fetch('/api/admin/config/technologies', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
        body: JSON.stringify({ name, type: isGraphicProject ? 'graphic' : 'common' }),
      });
      ...
      if (response.ok) {
        ...
        setError('Erreur lors de l\'ajout de la technologie');
      }
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                {categories.length === 0 && (
                  <p className="text-sm text-gray-500 italic">Chargement des catégories...</p>
                )}
              </div>
              {formData.category.length === 0 && categories.length > 0 && (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {technologies
                  .filter(t => isGraphicProject ? t.type === 'graphic' : t.type === 'common')
                  .map((tech) => (
                    <label key={tech.name} className="flex items-center space-x-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={formData.tech.includes(tech.name)}
                ...
                {technologies.length === 0 && (
                  <p className="text-sm text-gray-500 italic">Chargement des technologies...</p>
                )}
              </div>
